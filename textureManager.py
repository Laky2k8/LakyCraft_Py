...
class Texture_manager:
    def __init__(self, texture_width, texture_height, max_textures):
        ...
        gl.glTexParameteri(gl.GL_TEXTURE_2D_ARRAY, gl.GL_TEXTURE_MAG_FILTER,
                           gl.GL_NEAREST)  # disable texture filtering for magnification (return the texel that's nearest to the fragment's texture coordinate)
    ...
    def add_texture(self, texture):
        if not texture in self.textures:  # check to see if our texture has not yet been added
            ...
            texture_image = pyglet.image.load(
                f"textures/{texture}.png").get_image_data()  # load and get the image data of the texture we want
